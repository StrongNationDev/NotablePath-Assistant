...
for noisy_logger in ("httpx", "httpcore", "telegram", "telegram.ext", "urllib3"):
...
async def assessment_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("Continue", callback_data="START_ASSESSMENT")]])
    if update.message:
...
async def q_continue(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text and text.lower() != "continue":
        return QUESTION_CONTINUE

    if update.message:
        await update.message.reply_text("✅ Starting assessment...\n\n")
    elif update.callback_query and update.callback_query.message:
        await update.callback_query.message.reply_text("✅ Starting assessment...\n\n")
...
def _schedule_follow_up_job(context: ContextTypes.DEFAULT_TYPE, chat_id: int, stage: str, delay: timedelta) -> None:
    ...
    if not getattr(context.application, "job_queue", None):
        logger.warning("Job queue is not configured; skipping follow-up reminder for stage %s", stage)
        return
...
def main() -> None:
    ...
    assessment_handler = ConversationHandler(
        ...
        states={
            QUESTION_CONTINUE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, q_continue),
                CallbackQueryHandler(q_continue, pattern="^START_ASSESSMENT$"),
            ],
    ...
    app.add_handler(CallbackQueryHandler(q_continue, pattern="^START_ASSESSMENT$"))
    ...
    try:
        ...
        logger.error("2. Verify BOT_TOKEN in .env is valid and not expired")
        logger.error("3. Check if a firewall is blocking Telegram API (api.telegram.org)")
        logger.error("4. Try disabling any VPN/proxy temporarily")
        raise
